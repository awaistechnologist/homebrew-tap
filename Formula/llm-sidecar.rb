class LlmSidecar < Formula
  ...
  # Several dependencies are compiled: pydantic-core, cryptography, rpds-py and
  # primp are Rust; lxml is C against libxml2. Homebrew builds every resource
  # from source, so the toolchain is not optional.
  depends_on "rust" => :build
  depends_on "libxml2"
  depends_on "libxslt"

  # Written by hand rather than by `brew update-python-resources`, which
  # injects --uploaded-prior-to=P1D and so cannot see a release published in
  # the last 24 hours. That gate lives in the generator, not in a formula.
  ...
  def caveats
    <<~EOS
      ...
      To keep it running:  llm-sidecar service install
      To see what it sees: llm-sidecar status
    EOS
  end

  test do
    # `status` exits 1 when there is nothing to route to, which is the case on
    # a clean machine — so assert on what it reports, not the exit code.
